...
When working with Azure AKS clusters, use the `get_kubernetes_contexts` tool to list available contexts and the `set_kubernetes_context` tool to switch to the desired AKS cluster. After switching context, always use `get_flux_instance` to verify the Flux installation status on the selected cluster. Be aware of potential AKS-specific resources or configurations that might influence troubleshooting steps.

## ExternalSecrets Analysis

When troubleshooting issues related to secrets managed by External Secrets, follow these steps:

- Use the `get_kubernetes_api_versions` tool to find the correct apiVersion for `ExternalSecret` and `SecretStore` kinds.
- Use the `get_kubernetes_resources` tool to inspect the `ExternalSecret` and `SecretStore` resources. Analyze their `spec` and `status` fields for configuration issues or errors.
- Check the events of the `ExternalSecret` resource using `get_kubernetes_resources` to see if there are any reconciliation errors.
- If necessary, use the `get_kubernetes_resources` tool to find the pod(s) for the External Secrets controller (often in the `external-secrets` namespace) and use `get_kubernetes_logs` to retrieve their logs for detailed error information.
- Verify that the referenced `SecretStore` or `ClusterSecretStore` is correctly configured to connect to your secret provider (e.g., Azure Key Vault).
- Ensure the necessary permissions are granted to the AKS identity (pod identity or managed identity) for accessing secrets in the external store.